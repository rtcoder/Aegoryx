...
final readonly class UpdateCompanyAction
{
     ...
     */
    public function handle(CrmCompany $company, array $data, User $actor): CrmCompany
    {
        Gate::forUser($actor)->authorize('update', $company);

        return DB::transaction(function () use ($company, $data, $actor): CrmCompany {
            $before = $this->activityPayload($company);
            ...
            $this->activity->record(
                ...
                actorId: $actor->id,
                subject: $company,
                action: ActivityEntryAction::CrmCompanyUpdated,
                description: __('activity.crm_company_updated', ['company' => $company->name]),
                before: $before,
