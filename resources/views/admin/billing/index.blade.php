...
@section('subheading', __('admin.sections.billing'))

@section('content')
    <div class="grid gap-5 lg:grid-cols-2">
        <section class="ui-card">
            <div class="ui-card-header">
                <h2 class="ui-heading-2">{{ __('admin.billing.subscription_statuses') }}</h2>
            </div>
            <div class="ui-card-body space-y-3">
                ...
                @empty
                    <x-ui.empty-state :title="__('admin.billing.no_subscriptions')" />
                @endforelse
            </div>
        </section>
        ...
        <section class="ui-card">
            <div class="ui-card-header">
                <h2 class="ui-heading-2">{{ __('admin.billing.license_statuses') }}</h2>
            </div>
            <div class="ui-card-body space-y-3">
                ...
                @empty
                    <x-ui.empty-state :title="__('admin.billing.no_licenses')" />
                @endforelse
            </div>
        </section>
    </div>
    ...
    <section class="ui-card mt-5">
        <div class="ui-card-header">
            <h2 class="ui-heading-2">{{ __('admin.billing.recent_events') }}</h2>
            <p class="ui-body mt-1">{{ __('admin.billing.recent_events_description') }}</p>
        </div>
            ...
            <table class="ui-table">
                <thead>
                    <tr>
                        ...
                        <th>{{ __('admin.billing.provider') }}</th>
                        <th>{{ __('admin.billing.event_type') }}</th>
                ...
                <tbody>
                    @forelse ($billingEvents as $event)
                        <tr>
                            ...
                            <td class="text-right">
                                <a href="{{ route('admin.billing.events.show', $event) }}" class="ui-link">
                                    {{ __('admin.billing.details') }}
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6">
                                <x-ui.empty-state :title="__('admin.billing.no_events')" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
