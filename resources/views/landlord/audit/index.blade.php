...
@section('title', __('audit_view.audit_title').' | '.__('app.admin_title'))
@section('heading', __('audit_view.audit_title'))
@section('subheading', __('audit_view.audit_description'))

@section('content')
    <section class="ui-card">
        <div class="ui-card-header flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <h2 class="ui-heading-2">{{ __('audit_view.audit_log') }}</h2>
                <p class="ui-body mt-1">{{ __('audit_view.audit_help') }}</p>
            </div>

            <form method="GET" action="{{ route('landlord.audit.index') }}" class="flex gap-3">
                ...
                <tbody>
                    @forelse ($entries as $entry)
                        <tr>
                            ...
                            <td class="font-mono text-xs text-[var(--ui-text-muted)]">{{ $entry->actor_type }} #{{ $entry->actor_id ?? '-' }}</td>
                            <td class="font-mono text-xs text-[var(--ui-text-muted)]">{{ class_basename($entry->subject_type) }} #{{ $entry->subject_id ?? '-' }}</td>
                            <td class="font-mono text-xs text-[var(--ui-text-muted)]">{{ $entry->ip ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
