<div class="grid gap-5 lg:grid-cols-[360px_1fr]">
    <section class="rounded border border-neutral-800 bg-neutral-900 p-5">
        <h2 class="text-lg font-semibold">{{ __('features.create_title') }}</h2>

        <form wire:submit="createFeature" class="mt-5 space-y-4">
            <div>
                <label for="key" class="block text-sm font-medium text-neutral-300">{{ __('common.key') }}</label>
            ...
            <div>
                <label for="name" class="block text-sm font-medium text-neutral-300">{{ __('common.name') }}</label>
            ...
            <div>
                <label for="status" class="block text-sm font-medium text-neutral-300">{{ __('common.status') }}</label>
            ...
            <div>
                <label for="description" class="block text-sm font-medium text-neutral-300">{{ __('common.description') }}</label>
    ...
    <section class="overflow-hidden rounded border border-neutral-800 bg-neutral-900">
        <div class="border-b border-neutral-800 px-5 py-4">
            <p class="text-sm text-neutral-400">{{ __('features.index_note') }}</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-neutral-800 text-sm">
                <thead class="bg-neutral-950 text-left text-neutral-400">
                    <tr>
                        <th class="px-5 py-3 font-medium">{{ __('common.key') }}</th>
                        <th class="px-5 py-3 font-medium">{{ __('common.name') }}</th>
                        <th class="px-5 py-3 font-medium">{{ __('common.status') }}</th>
                        <th class="px-5 py-3 font-medium">{{ __('common.overrides') }}</th>
                    ...
                    @forelse ($features as $feature)
                        <tr wire:key="feature-{{ $feature->id }}">
                            ...
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('landlord.features.show', $feature) }}" wire:navigate class="text-sky-300 hover:text-sky-200">{{ __('common.open') }}</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-10 text-center text-neutral-400">{{ __('features.empty') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
