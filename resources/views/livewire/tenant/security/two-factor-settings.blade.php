<div class="space-y-5">
    ...
    <section class="ui-card">
        <div class="ui-card-header flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h2 class="ui-heading-2">{{ __('two_factor.tenant_2fa') }}</h2>
                <p class="ui-body mt-1">{{ __('two_factor.tenant_2fa_description') }}</p>
            </div>
            <x-ui.badge :variant="$user->hasTwoFactorEnabled() ? 'success' : 'warning'">
                {{ $user->hasTwoFactorEnabled() ? __('two_factor.enabled') : __('two_factor.disabled') }}
            </x-ui.badge>
        </div>

        <div class="ui-card-body space-y-5">
            @if ($user->hasTwoFactorEnabled())
                <p class="ui-body">{{ __('two_factor.enabled_notice', ['date' => $user->two_factor_confirmed_at?->format('Y-m-d H:i')]) }}</p>
                <x-ui.button type="button" wire:click="disable" variant="danger">
                    {{ __('two_factor.disable') }}
                </x-ui.button>
            @else
                <p class="ui-body">{{ __('two_factor.tenant_disabled_notice') }}</p>
                ...
                @else
                    ...
                    <form wire:submit="enable" class="max-w-md space-y-4">
                        <div>
                            <label for="tenant_two_factor_code" class="ui-label">{{ __('two_factor.confirm_code') }}</label>
                            <input id="tenant_two_factor_code" wire:model="code" autocomplete="one-time-code" class="ui-input mt-2">
                            @error('code')
