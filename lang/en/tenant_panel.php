<?php

return [
    'label' => 'Tenant panel',
    'active_tenant' => 'Active tenant',
    'modules' => 'Modules',
    'workspace_for' => 'Workspace for :tenant.',
    'workspace_overview_for' => 'Workspace overview for :tenant.',
    'dashboard_heading' => 'Tenant panel',
    'dashboard_description' => 'This shell is ready for tenant modules while keeping landlord-only details out of the workspace.',
    'context' => 'Tenant context',
    'not_enabled' => 'Not enabled for this tenant.',
    'enabled_features' => ':count enabled features',
    'login_description' => 'Sign in to the :tenant workspace.',
    'login_heading' => 'Tenant login',
    'module_placeholder' => 'Module implementation placeholder.',
    'sign_in' => 'Sign in',
    'sign_out' => 'Sign out',
    'roles' => [
        'owner' => 'Owner',
        'admin' => 'Admin',
        'member' => 'Member',
        'viewer' => 'Viewer',
    ],
    'users' => [
        'actions' => 'Actions',
        'create' => 'Create user',
        'create_description' => 'Add a user to the tenant and assign a role.',
        'create_title' => 'New user',
        'created' => 'User created.',
        'description' => 'Manage tenant user roles.',
        'email' => 'Email',
        'generated_password' => 'Generated one-time password',
        'last_owner_error' => 'The last owner cannot lose the owner role.',
        'name' => 'Name',
        'password' => 'Password',
        'password_help' => 'Leave empty to generate a strong password.',
        'read_only' => 'Read only',
        'role' => 'Role',
        'role_updated' => 'User role updated.',
        'save_role' => 'Save role',
        'title' => 'Users',
    ],
    'nav' => [
        'dashboard' => 'Dashboard',
        'dashboard_description' => 'Workspace overview',
        'cms' => 'CMS',
        'cms_description' => 'Pages, publishing, revisions',
        'crm' => 'CRM',
        'crm_description' => 'Contacts, companies, deals',
        'files' => 'Files',
        'files_description' => 'Private storage and downloads',
        'settings' => 'Settings',
        'settings_description' => 'Workspace configuration',
        'security' => 'Security',
        'security_description' => '2FA and account protection',
        'users' => 'Users',
        'users_description' => 'Roles and access',
        'activity' => 'Activity',
        'activity_description' => 'Tenant event history',
    ],
];
