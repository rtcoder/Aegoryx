<?php

return [
    'label' => 'Панель tenant',
    'active_tenant' => 'Активный tenant',
    'modules' => 'Модули',
    'workspace_for' => 'Рабочая область для :tenant.',
    'workspace_overview_for' => 'Обзор рабочей области для :tenant.',
    'dashboard_heading' => 'Панель tenant',
    'dashboard_description' => 'Эта оболочка готова для модулей tenant и скрывает детали, доступные только landlord.',
    'context' => 'Контекст tenant',
    'not_enabled' => 'Не включено для этого tenant.',
    'enabled_features' => ':count включенных функций',
    'login_description' => 'Войдите в рабочую область :tenant.',
    'login_heading' => 'Вход tenant',
    'module_placeholder' => 'Место для реализации модуля.',
    'sign_in' => 'Войти',
    'sign_out' => 'Выйти',
    'roles' => [
        'owner' => 'Owner',
        'admin' => 'Admin',
        'member' => 'Member',
        'viewer' => 'Viewer',
    ],
    'users' => [
        'actions' => 'Действия',
        'create' => 'Создать пользователя',
        'create_description' => 'Добавьте пользователя в tenant и назначьте роль.',
        'create_title' => 'Новый пользователь',
        'created' => 'Пользователь создан.',
        'description' => 'Управление ролями пользователей tenant.',
        'email' => 'Email',
        'generated_password' => 'Сгенерированный одноразовый пароль',
        'last_owner_error' => 'Последний owner не может потерять роль owner.',
        'name' => 'Имя',
        'password' => 'Пароль',
        'password_help' => 'Оставьте пустым, чтобы сгенерировать сильный пароль.',
        'read_only' => 'Только чтение',
        'role' => 'Роль',
        'role_updated' => 'Роль пользователя обновлена.',
        'save_role' => 'Сохранить роль',
        'title' => 'Пользователи',
    ],
    'nav' => [
        'dashboard' => 'Панель',
        'dashboard_description' => 'Обзор рабочей области',
        'cms' => 'CMS',
        'cms_description' => 'Страницы, публикация, ревизии',
        'crm' => 'CRM',
        'crm_description' => 'Контакты, компании, сделки',
        'files' => 'Файлы',
        'files_description' => 'Приватное хранилище и загрузки',
        'settings' => 'Настройки',
        'settings_description' => 'Конфигурация рабочей области',
        'security' => 'Безопасность',
        'security_description' => '2FA и защита учетной записи',
        'profile' => 'Мой профиль',
        'profile_description' => 'Язык и предпочтения пользователя',
        'users' => 'Пользователи',
        'users_description' => 'Роли и доступ',
        'activity' => 'Активность',
        'activity_description' => 'История событий tenant',
    ],
];
